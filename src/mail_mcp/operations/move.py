...
class EmailMove:
    ...
    def _validate_uids(self, uids: Union[int, List[int]]) -> str:
        """
    ...
    def copy_email(
        ...
        try:
            ...
            status = response[0]
            if status not in ('OK', b'OK') and not isinstance(status, int):
                error_msg = response[1][0].decode('utf-8', errors='replace') if response[1] and response[1][0] else 'Unknown error'
                
                if 'does not exist' in error_msg.lower():
                    raise EmailMoveError(f"Destination folder '{destination_folder}' does not exist")
    ...
    def delete_email(
        ...
        folder: str,
        uids: Union[int, List[int]],
        expunge_immediately: bool = False
    ) -> bool:
        """
...
def move_email(connection, source_folder: str, uids: Union[int, List[int]], dest_folder: str) -> bool:
    """Move emails between folders."""
    mover = EmailMove(connection)
    return mover.move_email(source_folder, uids, dest_folder)


def copy_email(connection, source_folder: str, uids: Union[int, List[int]], dest_folder: str) -> bool:
    """Copy emails to another folder."""
    mover = EmailMove(connection)
    return mover.copy_email(source_folder, uids, dest_folder)


def delete_email(connection, folder: str, uids: Union[int, List[int]]) -> bool:
    """Delete emails."""
    mover = EmailMove(connection)

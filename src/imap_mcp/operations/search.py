...
class EmailSearch:
    ...
    def _build_search_criteria(self, conditions: Dict[str, Any]) -> List[str]:
        ...
        for key, value in conditions.items():
            ...
            # 包括正向和反向 flag
            flag_keys = ['seen', 'unseen', 'answered', 'unanswered', 'flagged', 'unflagged', 
                         'deleted', 'undeleted', 'draft', 'undraft', 'recent', 'unread', 'all']
    ...
    def search_emails(
        ...
        try:
            ...
            if status not in ('OK', b'OK') and not isinstance(status, int):
                error_msg = select_result[1][0].decode('utf-8', errors='replace') if select_result[1] else 'Unknown error'
